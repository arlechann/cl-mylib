...
`pa` / `pa*` では `:$0`, `:$1`, ... を位置プレースホルダとして使います。`:$*` は残りの引数全体を 1 つのリストとして渡し、`:$@` は残りの引数をその位置に展開します。`pa*` はプレースホルダでない式を関数生成時に評価します。

Macro: **fn** `&body body`

Macro: **fn\*** `&body body`

`fn` / `fn*` では `:$0`, `:$1`, ... を位置プレースホルダとして使います。`:$*` は残りの引数全体を 1 つのリストとして参照します。`fn*` はプレースホルダでない式を関数生成時に評価します。
